...
function insertMessage (req, res) {
	...
	pool.query(sql, values, function(err, data) {
		if (err) {
			...
			sql = 'SELECT * FROM (SELECT message.id, sender_id, receiver_id, message, timestamp from message WHERE sender_id = $1::integer AND receiver_id = 2::integer UNION SELECT message.id, sender_id, receiver_id, message, timestamp from message WHERE sender_id = $2::text AND receiver_id = $1::integer) tmp ORDER BY tmp.timestamp;'; 
			values = [req.session.contactusername, req.session.userid];
			pool.query(sql, values, function(err, data) {
				if (err) {
					console.log(err);
